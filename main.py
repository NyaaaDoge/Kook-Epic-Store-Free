...
async def epic(msg: Message, command: str = None, *args):
    ...
    # TODO 如果是服务器管理员才能执行操作，目前不会写获取用户的permissions
    try:
        if command in ['free', '免费', '免费游戏']:
            if not any(args):
            ...
            # 开启订阅
                ...
                if any(db_Channel):
                    ...
                    if db_Channel[6] == 0:
                        ...
                        await msg.reply("订阅Epic商店限时免费商品推送功能 **[:green_square:开启]** 成功", type=MessageTypes.KMD)
                    else:
                ...
                # 频道不在数据库中，执行插入操作
                    ...
                    if not any(db_Channel):
                        ...
                        if insert_flag:
                            ...
                            for item in free_items:
                                ...
                                if not item[13] == '':
                                    ...
                                    if db_end_time > now_time:
                                        # 进行推送
                                        await bot.client.send(target=current_channel, type=MessageTypes.CARD,
                            ...
                            await msg.reply(":yellow_square:服务器新增推送频道失败，因为目前频道已加入过推送功能！", type=MessageTypes.KMD)
                    # 同一个服务器
                    else:
...
频道ID：**{db_Channel[4]}**""", type=MessageTypes.KMD)
                ...
                if unsub_flag:
                    await msg.reply("订阅Epic商店限时免费商品推送功能 **[:black_large_square:关闭]** 成功", type=MessageTypes.KMD)
                else:
...
async def getFreeGames():
    ...
    try:
        ...
        if flag_insert:
            logging.info(f"Insert {len(free_items)} item(s) info into the table successfully.")
        else:
            logging.info(f"The {len(free_items)} item(s) inserted info the table fail, probably because of duplicate.")
    except Exception as e:
        logging.error(e, exc_info=True)
